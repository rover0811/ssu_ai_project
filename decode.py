...
parser.add_argument('--modeltype', type=str, default="base")
...
if args.loadfrom != '':
    biasing_model = torch.load(args.loadfrom, weights_only=False)   # 모델 로드하지 못하는 오류 해결
    biasing_model.eval()
...
tokenizer = whisper.tokenizer.get_tokenizer(model.is_multilingual, language="ko")   # tokenizer 한글로 설정정
...
for idx, data in enumerate(testloader):
    ...
    options = whisper.DecodingOptions(
        ...
        origtree=origtree,
        fp16=False,
#        shallowfusion=shallowfusion,  # gpt2 관련 주석 처리
#        useGPT=useGPT,
#        GPT2=GPTmodel,
        lm_weight=args.lm_weight,
#        GPT2tokenizer=GPTtokenizer,    # gpt2 관련 주석 처리
